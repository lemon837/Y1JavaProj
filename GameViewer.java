...
public class GameViewer implements MouseListener {
    ...
    public void drawGameOutcome() {
        if (bd.getFoundItemsList(selectedItems).size() == selectedItems.length) {
            sc.drawGameWon(0, 0);
        }
    ...
    public void refreshBoard(int xLastClicked, int yLastClicked) {
        //1
        bd.getFoundItemsList(selectedItems);
    ...
    public int takeTurn(int x, int y) {
        ...
        else if (bd.isSearched(x, y) || bd.isFoundItem(x, y)) {
            return turnsRemaining;
        }
        else if (bd.isVacant(x, y)) {
            bd.setSearched(x, y);
            turnsRemaining -= 1;
        }
        else if (bd.isLostItem(x, y)) {
            bd.setFoundItem(x, y);
        }
        refreshBoard(x, y);
        return turnsRemaining;
    ...
    public void mousePressed(MouseEvent e) {
        int [] clicked = getNearestPiece(e.getX(), e.getY());
        
        if (e.getX() >= 0 && e.getX() <= 150 && e.getY() >= brdSize + scoreboardSize - 50 && e.getY() <= brdSize + scoreboardSize) {
            restartGame();
        }
        else { takeTurn(clicked[0], clicked[1]); } 
    }
