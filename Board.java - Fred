...
public class Board {
    ...
    * Check if a given piece has been searched by the player (but does not contain
    ...
    public boolean checkForFoundItem(Item item) {
        ...
        int[][] board2 = new int[getBoardSize()][getBoardSize()];
        // Add a 1 into the new board for item pieces and a 0 for an empty space
        for (int i = 0; i < board[0].length; i++){
            for (int j = 0; j < board.length; j++){
                if (board[i][j] == Piece.VACANT) {
                    board2[i][j] = 0; }
                else if (board[i][j] == Piece.SEARCHED) {
                    board2[i][j] = 0; }
                else if (board[i][j] == Piece.LOSTITEM) {
                    board2[i][j] = 1; }
                else if (board[i][j] == Piece.FOUNDITEM) {
                    board2[i][j] = 1; }
                }
            }
        
        // Check for the existence of the Shape 2D Array within the Board2 2D Array
        for (q = 0; q < board2.length; q++) {
            for (t = 0; t < board2[q].length; t++) { 
                if (board2[q][t] == shape[0][0])
                    if (findMatch(board2, shape, q, t)) {
                        return true;
                    }
            } 
        }
        ...
        function findMatch(board2, shape, q, t) {
        var b = true;
        for (k = q; k < shape.length; k++) {
        for (n = t; n < y[k].length; n++) {
            if (y[k - i][n - j] != x[k][n]) {
                b = false;
                break;
                }
